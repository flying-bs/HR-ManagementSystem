...
public class DepDAO extends HibernateDaoSupport {
	...
	public List<Dep> findByExample(Dep instance) {
		log.debug("finding Dep instance by example");
		try {
			List<Dep> results = getHibernateTemplate()
	...
	public List<Dep> findByDepname(Object depname) {
	...
	public Dep merge(Dep detachedInstance) {
		...
		try {
			Dep result = getHibernateTemplate().merge(detachedInstance);
