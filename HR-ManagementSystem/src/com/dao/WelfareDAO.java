...
public class WelfareDAO extends HibernateDaoSupport {
	...
	public List<Welfare> findByExample(Welfare instance) {
		log.debug("finding Welfare instance by example");
		try {
			List<Welfare> results = getHibernateTemplate()
	...
	public List<Welfare> findByWname(Object wname) {
	...
	public Welfare merge(Welfare detachedInstance) {
		...
		try {
			Welfare result = getHibernateTemplate().merge(
					detachedInstance);
